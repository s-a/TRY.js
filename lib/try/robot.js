(function(engine, toastr) {
	...
	var fadeTo = function function_name (opacity, time, done) {
		...
		var all = obj.material.materials.length;
	...
	var getMovedWallCollisionItems = function  (wall, distance) {
		var movable = wall.userData.movable;
		var hex  = 0xff0000;
		var testWall  =  new THREE.BoundingBoxHelper( wall, hex );
		testWall.material.side = THREE.DoubleSide;
		testWall.update();
 
		if (engine.debugMode){
			testWall.material.color.setHex( Math.random() * 0xffffff ); 
			engine.scene.add(testWall); // debug
			window.debugBoxen.push(testWall);
		}
		testWall.position.copy(movePosition(wall.position, distance)); 
 
		var collidableMeshList = [];
		for (var i = 0; i < engine.wall.meshList.length; i++) {
			var mesh = engine.wall.meshList[i];
			if (mesh.id !== wall.id){
				collidableMeshList.push(mesh);
			}
		}
		return engine.getCollisionObjects(testWall, collidableMeshList);
	}
	...
	Bot.prototype.push = function(count, onComplete) {
		...
		var doPush = function (done) {
			...
			if (wall){
				if (wall.userData.movable === true){
					var movedWallCollisionItems = getMovedWallCollisionItems(wall, distance);
					// wall = testMove.movedWall; // FIXME: need this reference? fix it to move multiple walls . 
						 
					if (movedWallCollisionItems.length > 0){
					...
					toastr.error("Wall not mavable. Program abort.");
				}
			} else {
