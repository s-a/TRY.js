...
(function(THREE) {
	...
	createjs.Sound.registerSound({src:"snd/teleport.mp3", id:"teleport"});
	createjs.Sound.registerSound({src:"snd/refuel.mp3", id:"refuel"});
...
/*		if (selected){
	...
	function addWall(meta) {
		...
		wall.userData.setMovable = function  (movable) {
			meta.movable = movable;
			wall.userData.movable = meta.movable;
		};
	...
	function boxesAreEqual (box1, box2) {
		var result = (
			box1.position.x === box2.position.x &&
 			box1.position.y === box2.position.y &&
 			box1.position.z === box2.position.z &&
 			box1.geometry.height === box2.geometry.height &&
 			box1.geometry.width === box2.geometry.width &&
 			box1.geometry.depth === box2.geometry.depth
		);
		return result;
	}

	function _getCollisionObjects (mesh, collidableMeshList) {
		var originPoint = mesh.position.clone();

/*		for (var vertexIndex = 0; vertexIndex < mesh.geometry.vertices.length; vertexIndex++) {
			var ray = new THREE.Raycaster( mesh.position, mesh.geometry.vertices[vertexIndex] );
			var collisionResults = ray.intersectObjects( collidableMeshList );
	...
	function getCollisionObjects (box, collidableMeshList) {
		var result = [];
		...
		for (var i = 0; i < collidableMeshList.length; i++) {
			...
			var bbox = new THREE.BoundingBoxHelper( m, hex );
			bbox.update();
			bbox.position.copy((m.position));
			if (engine.debugMode){
				scene.add(bbox);
				window.debugBoxen.push(bbox);

			}
			//console.log(box.box.isIntersectionBox(bbox.box) , box.box.containsBox(bbox.box) , boxesAreEqual(box, bbox), box.position, bbox.position);
			if (
				bbox.box.isIntersectionBox(box.box) || box.box.isIntersectionBox(bbox.box) || box.box.containsBox(bbox.box) || bbox.box.containsBox(box.box) || boxesAreEqual(box, bbox)
			){
				result.push(m);
			}
		}
		if (result.length === 0){
			...
			result = _getCollisionObjects(box, collidableMeshList);
		}
		return result;
	}
...
/*				var testRunner = function() {
					...
					if (index===packages.length-1){
						...
						if (packages[index].test) {
							log("Loading " + packages[index].name + "...") ;
							engine.channel.load(index, function(p) {
								toastr.clear();
								log("Run program \"" + packages[index].name + "\"...");
								window.robot.program.run(function  () {
									// cool down client before test
									log("Cooling down... Testing...");
									window.setTimeout(function() {
										toastr.clear();
				...
				var addTest = function(idx, name) {
					describe(name, function() {
						var sourceCode = ";";

						before("should load tests", function(done) {
							window.engine.channel.dryLoad(idx, function(src) {
								sourceCode = src;
								src.length.should.be.above(0);
								done();
							});
						});

						it("should execute program", function(done) {
						...
						it("should pass the tests", function(done) {
							this.timeout(1000*60*10);
							var f = new Function("done", sourceCode + "; done();");
							var funTest = $.proxy(f, window.engine);
							funTest(done);
						});
				...
				describe("engine.channel.list", function() {
				    it("should contain packages", function( done ) {
			      		packages.length.should.be.above(0);
						for (var i = 0; i < packages.length; i++) {
							var pack = packages[i];
							if (pack.program && pack.test) {
								addTest(i, pack.name);
							}
						}
	...
	window.showApplication = function function_name (argument) {
		window.initialzeGUI();
		$(".entry-open").hide();
		$("#ThreeJS").show();
	}
	...
	$(function() {
        ...
        if (navigator.userAgent.indexOf('SlimerJS') !== -1) {
			console.log("SlimerJS testing warmup...");
			window.setTimeout(function() {
				console.log("SlimerJS testing...", window.engine);
				window.engine.test();
			}, 1000);
        }
	});
