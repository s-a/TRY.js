...
(function(THREE) {
	...
	function resetSelectedStation (station) {
		for (var i = 0; i < stationMeshList.length; i++) {
	    	...
	    	item.material.color.set( new THREE.Color( 0x009900 ) );
		}

	    station.material.color.set( new THREE.Color( 0x000000 )  );
	}
...
/*		if (selected){
	...
	function addStation(meta) {
		...
		var wireMaterial = new THREE.MeshBasicMaterial( { color: 0x009900, wireframe:false , side: THREE.DoubleSide, opacity:0.44, transparent:true, } );
	...
	function init(){
		...
		////////////
		var cubeGeometry = new THREE.CubeGeometry(50,50,50,1,1,1);
		var wireMaterial = new THREE.MeshBasicMaterial( { color: 0xff0000, wireframe:false , side: THREE.DoubleSide, opacity:0.44, transparent:true, } );
	...
	if (!window.engine) {
		window.engine = {
			...
			loadEnvironment : function(meta) {
				...
				window.originalStaionPositions = meta.stations;
				if (meta.flowers){
