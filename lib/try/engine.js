...
/*		if (selected){
	...
	function resetSelectedFlower (f) {
		...
		if (!meta){
			...
			switch(window.robot.direction().val()){
				case "S":
					...
					f.position.y = 0;
					f.position.z = android.position.z + (window.movementSetup.distance * window.movementSetup.repeats)-5;
					break;
				default:
					toastr.info("todo");
			}
	...
	function collision (obj) {
		...
		return __collision(obj, collidableMeshList) || _collision(bbox,collidableFlowerList);
	} 
	...
	if (!window.engine) {
		window.engine = {
			...
			loadEnvironment : function(meta) {
				...
				window.engine.originalBotPosition = android.position.clone();
				window.engine.originalBotRotation = android.rotation.clone();
				window.orginalBotCapacityCurrent = meta.robot.capacity;
