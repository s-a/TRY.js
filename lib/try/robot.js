(function(engine, toastr) {
	...
	Bot.prototype.push = function(count, onComplete) {
		...
		function doPush (done) {
			...
			bbox.update();
			var result = engine.getWallCollisionItem(bbox);
			if (result){
				var testWall  = new THREE.BoundingBoxHelper( result, hex );
				testWall.update();
				var targetPosition = result.position.clone();
				switch (robot.direction().val()){
					case "S":
						testWall.position.z += distance;
						targetPosition.z += distance;
						break;
					case "W":
						testWall.position.x -= distance;
				...
				if (!movedWallCollisionItem){
					var tween = new TWEEN.Tween(result.position).to(targetPosition, window.robotConfigs.speed);
					tween.easing(TWEEN.Easing.Quadratic.In);
					tween.onComplete(function() {
						console.log(robot.direction().val());
						window.robotConfigs.energy.current--;
						window.robotConfigs.energy.current--;
						window.robotConfigs.energy.current--;
						window.robotConfigs.energy.current--;
					...
					toastr.error("Move wall collision.");
					return false;
				}
			} else {
				toastr.error("Nothing to push in front of me.");
			}
			return result;
		}
