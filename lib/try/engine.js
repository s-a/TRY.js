...
*/
			...
			test : function() {
				...
				// TODO gerator for a whole bunch if all testsuites! :)
				...
				describe("package(" + idx + ")", function() {
					var pack = null; 
					var sourceCode = ";";

					it("should load tests", function(done) {
						pack = packages[idx];
					...
					it("should execute program", function(done) {
						...
						window.robot.program.run(function() {
							...
							/*var f = new Function("done", scr);
							funTest = $.proxy(f, engine);
							funTest();*/
							window.robot.sensor.position().x.should.be.equal(0);
							done(); 
						});
					});
...
/*						describe(pack.name, function() {
						    it("should execute package", function(done) {
								window.engine.channel.dryLoad(idx, function(p) {
									window.robot.program.run(function  () {
										// cool down client before test
										log("Cooling down... Testing...");
										window.toastr.clear();
