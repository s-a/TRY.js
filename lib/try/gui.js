(function(dat, robot) {
	...
	var parameters = {
		...
		charge: function() {
			if (!robot.charge()){
				alert("Charge does only work within a energy box.");
			}
		},
		get: function() {
			if (!robot.get()){
				alert("Here is nothing to pick up.");
			}
		},
		put: function() {
			if (!robot.put()){
				alert("I have nothing to unload or it is not possible to unload something here because there is no space.");
			}
		},
		run: function() {
			robot.run(10);
	...
	f4.add( parameters, 'share_on_github' ).name("Share");
