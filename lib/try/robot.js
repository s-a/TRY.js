(function(engine) {
	...
	var refreshRobotEnergyStatus = window.refreshRobotEnergyStatus = function(charging) {
		...
		$("#robot__energy").attr("icon", ico + icon).css("color", color).attr("title", window.robotConfigs.energy.current);
	};
	...
	Bot.prototype.tryGet = function() {
		...
		var direction = "forward";
					var hex  = 0xff0000;
			var bbox = new THREE.BoundingBoxHelper( engine.android, hex );
		switch(direction) {
		    case "forward":
				bbox.translateZ( distance  );
		        break;
		    case "backward":
				bbox.translateZ( -distance );
		        break;
		...
		var result = engine.flowerCollision(bbox);
		return !result;
	};
